...
pub async fn start_deep_video_analysis(
    ...
    let (task_id, abort_registration) = with_deep_video_lifecycle_lock(async {
        ...
        if let Err(error) = get_task_queue().start_task_by_id(&task_id).await {
            DEEP_VIDEO_ABORT_HANDLES.lock().remove(&task_id);
            return Err(error.to_string());
        }

        get_task_queue()
            .update_task_progress_by_id(&task_id, 0.1)
            .await;
        emit_task_progress(&app, &task_id, 0.1, "running");

        Ok((task_id, abort_registration))
    })
    .await?;
...
mod tests {
    ...
    async fn lifecycle_lock_serializes_startup_and_cancel_sections() {
        ...
        let guard = DEEP_VIDEO_LIFECYCLE_MUTEX.lock().await;
        ...
        assert!(matches!(futures_util::poll!(cancel.as_mut()), Poll::Pending));
        ...
        assert_eq!(recorded, vec!["cancel-enter"]);
    }
}
